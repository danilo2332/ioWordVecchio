...
public class UpdatesServlet extends HttpServlet {
		...
		try {
			...
			if (changes.size() == QUERY_LIMIT) {
				request.setAttribute("warning",
						"Result set limited to " + QUERY_LIMIT + " results. Please narrow your query.");
			}
	...
	/**
	 * Construct a list of Updates from the database using the given query
	 * parameters. All parameters are optional where a null or empty value excludes
	 * it from the filter.
	 *
	 * @param start
	 *            - the fully qualified date & time to start getting updates from.
	 * @param end
	 *            - the fully qualified date & time to stop getting updates at.
	 * @param otype
	 *            - the otype of the document types to get updates for.
	 * @param oid
	 *            - the oid of the document to get updates for.
	...
	private TreeMap<Date, TreeMap<Date, ArrayList<Change>>> structureChanges(List<Change> changes) {
		...
		TreeMap<Date, TreeMap<Date, ArrayList<Change>>> structuredChanges = new TreeMap<Date, TreeMap<Date, ArrayList<Change>>>(
